...
import { Campaign, Department, Teacher } from "../types.js";

interface CampaignManagerProps {
  campaigns: Campaign[];
  teachers: Teacher[];
  departments: Department[];
  onAddCampaign: (campaign: { title: string; description: string; deadline: string; excludedTeacherIds?: string[] }) => Promise<void>;
...
export default function CampaignManager({
  ...
  const buildExcludedTeacherIds = () =>
    teachers.filter(t => !selectedDepartmentIds.includes(t.departmentId)).map(t => t.id);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (departments.length > 0 && selectedDepartmentIds.length === 0) {
      setErrorMsg("Vui lòng chọn ít nhất 1 tổ chuyên môn.");
      return;
    }

    setSaving(true);
    try {
      await onAddCampaign({ title, description, deadline, excludedTeacherIds: buildExcludedTeacherIds() });
      resetForm();
    } catch (err: any) {
  ...
  const startEdit = (camp: Campaign) => {
    ...
    const excluded = new Set(camp.excludedTeacherIds || []);
    const selected = departments.filter(d => {
      const deptTeachers = teachers.filter(t => t.departmentId === d.id);
      return deptTeachers.length === 0 || deptTeachers.some(t => !excluded.has(t.id));
    }).map(d => d.id);
    setSelectedDepartmentIds(selected.length ? selected : departments.map(d => d.id));
    setShowAddForm(false);
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await onUpdateCampaign(editingCampId, { title, description, deadline, excludedTeacherIds: buildExcludedTeacherIds() });
      resetForm();
    } catch (err: any) {
          ...
          <form onSubmit={editingCampId ? handleUpdate : handleCreate} className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                <label className="text-xs font-bold text-gray-700">{"Ch\u1ecdn t\u1ed5 CM"}</label>
                <button
                  type="button"
                  onClick={() => setSelectedDepartmentIds(
                    selectedDepartmentIds.length === departments.length ? [] : departments.map(d => d.id)
                  )}
                  className="text-[11px] font-semibold text-emerald-700 hover:text-emerald-800"
                >
                  {selectedDepartmentIds.length === departments.length ? "B\u1ecf ch\u1ecdn t\u1ea5t c\u1ea3" : "Ch\u1ecdn t\u1ea5t c\u1ea3"}
                </button>
              </div>
              <div className="flex flex-wrap gap-2">
                {departments.map(dept => {
                  const selected = selectedDepartmentIds.includes(dept.id);
                  return (
                    <button
                      key={dept.id}
                      type="button"
              ...
              <p className="text-[11px] text-gray-400">{"Ch\u1ec9 gi\u00e1o vi\u00ean thu\u1ed9c c\u00e1c t\u1ed5 \u0111\u01b0\u1ee3c ch\u1ecdn m\u1edbi hi\u1ec3n th\u1ecb trong \u0111\u1ee3t n\u1ed9p n\u00e0y."}</p>
            </div>
