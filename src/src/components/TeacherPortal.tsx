...
export default function TeacherPortal({
  ...
  const eligibleTeachers = useMemo(() => {
    ...
    return teachers.filter(t => !excluded.has(t.id));
  }, [teachers, currentCampaign]);

  const eligibleDepartmentIds = useMemo(
    () => new Set(eligibleTeachers.map(t => t.departmentId)),
    [eligibleTeachers]
  );

  const availableDepartments = useMemo(
    () => departments.filter(d => eligibleDepartmentIds.has(d.id)),
  ...
  }, [eligibleTeachers, selectedDeptId, searchTeacher]);

  const submittedTeacherIds = useMemo(() => new Set(
    submissions.filter(s => s.campaignId === selectedCampaignId).map(s => s.teacherId)
  ), [submissions, selectedCampaignId]);

  const submittedCount = eligibleTeachers.filter(t => submittedTeacherIds.has(t.id)).length;
        ...
        <div className="bg-white border border-emerald-100 rounded-3xl shadow-xl p-8 text-center">
          ...
          <div className="mt-7 flex justify-center">
            <button
              ...
              className="px-8 py-3 bg-emerald-700 hover:bg-emerald-800 rounded-xl text-sm font-bold text-white shadow-sm transition-colors"
            >
              Thoát
            </button>
          </div>
        ...
        <div className="bg-[#007A48] text-white p-6 sm:p-8 rounded-t-3xl">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-5">
            ...
            <div className="bg-[#005c36] rounded-2xl px-5 py-4 text-center shrink-0">
              <div className="text-3xl font-black text-yellow-300">{submittedCount}/{eligibleTeachers.length}</div>
                    ...
                    <div className="absolute left-0 right-0 top-full z-[9999] mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden">
                      ...
                      <div className="max-h-72 overflow-auto p-2">
                        {filteredTeachers.map(t => {
                          const hasSubmitted = submittedTeacherIds.has(t.id);
                          return (
                            <button
                              key={t.id}
                              type="button"
                              onClick={() => { setSelectedTeacherId(t.id); setShowTeachers(false); setSearchTeacher(""); }}
                              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors ${hasSubmitted ? "bg-emerald-50 hover:bg-emerald-100" : "hover:bg-slate-50"}`}
                            >
                              <span className={`h-5 w-5 rounded border flex items-center justify-center ${selectedTeacherId === t.id ? "bg-emerald-600 border-emerald-600 text-white" : "border-slate-300 bg-white"}`}>
                                {selectedTeacherId === t.id && <Check className="h-3.5 w-3.5" />}
                              </span>
                              <span className="flex-1 text-sm font-semibold text-slate-800">{t.name}</span>
                              <span className={`text-xs font-semibold ${hasSubmitted ? "text-emerald-700" : "text-red-500"}`}>
